...
System_Boundary(cis_api_boundary, "CIS API") {
    ...
    Container(infrastructure_mysql, "Infrastructure (MySQL)", ".NET Library", "Implements MySQL data access.")
    Container(infrastructure_mongodb, "Infrastructure (MongoDB)", ".NET Library", "Implements MongoDB data access.")
}

System_Ext(user, "Authenticated User")
System_Ext(user_api, "Users API")
SystemDb_Ext(mysql_db, "MySQL Database", "MySQL", "Stores legacy data.")
SystemDb_Ext(mongodb_db, "MongoDB Database", "MongoDB", "Stores new data.")
System(data_migration, "Data Migration Service", ".NET Console App", "Migrates data from MySQL to MongoDB.")
...
Rel(application, infrastructure_mysql, "Uses (via DI)")
Rel(application, infrastructure_mongodb, "Uses (via DI)")

Rel(infrastructure_mysql, mysql_db, "Reads/writes to")
Rel(infrastructure_mongodb, mongodb_db, "Reads/writes to")
Rel(infrastructure_mysql, domain, "Implements interfaces from")
Rel(infrastructure_mongodb, domain, "Implements interfaces from")

Rel(data_migration, mysql_db, "Reads from", "JDBC/ODBC")
Rel(data_migration, mongodb_db, "Writes to", "MongoDB Driver")
@enduml
```
